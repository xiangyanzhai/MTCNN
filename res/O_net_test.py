...
def landmarks2landmark2(bboxes, landmarks):
    ...
    landmarks = landmarks.view(-1, 5, 2)

    wh = wh.view(-1, 1, 2)
    xy1 = xy1.view(-1, 1, 2)
    landmarks = landmarks * wh + xy1
...
class P_net_test():
    ...
    def get_res(self, x):
        ...
        tanchors = self.get_R_net_res(x, tanchors[..., :4])

        if tanchors.shape[0] == 0:
            return tanchors
        res = self.get_O_net_res(x, tanchors[..., :4])
        res[..., :14] = res[..., :14]
        return res

    def get_O_net_res(self, x, tanchors):
        h, w = x.shape[2:]
        ...
        xx = roialign_48(x, roi)
        xx = (xx - 127.5) / 128.0
        net_logits, net_loc, net_landmarks = self.O_net(xx)
        # net_logits = net_logits.view(-1, 2)
        # net_loc = net_loc.view(-1, 4)
        # net_landmarks = net_landmarks.view(-1, 10)
        net_landmarks = torch.sigmoid(net_landmarks)

        score = F.softmax(net_logits, dim=-1)[..., 1]

        inds = score >= self.config.O_net_conf_thresh
        if inds.sum() == 0:
            return cuda(torch.zeros((0, 5)))
        score = score[inds]
        net_loc = net_loc[inds]
        net_landmarks = net_landmarks[inds]
        ...
        landmarks = landmarks2landmark2(bboxes, net_landmarks)
        hw = bboxes[:, 2:4] - bboxes[:, :2]
        inds = hw >= self.config.roi_min_size[2]
        inds = inds.all(dim=1)
        if inds.shape[0] == 0:
            return cuda(torch.zeros((0, 14)))
        bboxes = bboxes[inds]
        score = score[inds]
        ...
        keep = nms(bboxes, self.config.O_net_iou_thresh, "Minimum")
        bboxes = bboxes[keep]
        landmarks = landmarks[keep]
        score = score[keep]
        score = score.view(-1, 1)
        return torch.cat([bboxes, landmarks, score], dim=-1)
    ...
    def test(self, model_file):
        ...
        self.R_net.eval()
        cuda(self.R_net)
        self.O_net.load_state_dict(
            torch.load(model_file[2], map_location='cpu'))
        self.O_net.eval()
        cuda(self.O_net)
        ...
        for i in range(len(test_dir)):
            ...
            with codecs.open(w_file, 'w') as f:
                for file in test_dir[i]:
                    ...
                    # if res.shape[0]>0:
                    #     draw_gt(t_img, res[..., :14], ellips[file])
                    # draw_gt(t_img, res[:, :14])
...
if __name__ == "__main__":
    ...
    O_net_model_file = '/home/zhai/PycharmProjects/Demo35/MTCNN/train/models/O_net_45000_1.pth'

    model_file = [P_net_model_file, R_net_model_file, O_net_model_file]
