...
class O_net_train():
    ...
    def get_train_data(self, x, bboxes, num_b, H, W, tanchors, num_A):
        ...
        xx = (xx - 127.5) / 128.0
        O_net_logits, O_net_loc, O_net_landmarks = self.model(xx)
        # O_net_logits = O_net_logits.permute(0, 2, 3, 1).view(-1, 2)
        # O_net_loc = O_net_loc.permute(0, 2, 3, 1).view(-1, 4)
        # O_net_landmarks = O_net_landmarks.permute(0, 2, 3, 1).view(-1, 10)
    ...
    for epoch in range(epochs):
        for imgs, bboxes, num_b, num_H, num_W, anchors, num_A in dataloader:
            ...
            if (step <= 10000 and step % 1000 == 0) or step % 5000 == 0 or step == 1:
                torch.save(model.state_dict(), './models/O_net_%d_1.pth' % step)
            if step >= 90010 * x:
                flag = True
                break
        if flag:
            break
    torch.save(model.state_dict(), './models/O_net_final_1.pth')
...
if __name__ == "__main__":
    ...
    files = [path + 'O_net_train_wider_1.pkl']
    config = Config(files, lr=0.01, weight_decay=0.0001, scale=0.79,
    ...
    step =20000
    x = 0.5
    model_file = '/home/zhai/PycharmProjects/Demo35/MTCNN/train_3/models/O_net_20000_1.pth'
    O_train = O_net_train()
    train(O_train, config, step, x, model_file=model_file)
