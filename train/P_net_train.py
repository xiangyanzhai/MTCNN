...
class P_net_train():
    ...
    def get_train_data(self, x, bboxes, num_b, H, W):
        ...
        while True:
            ...
            if n_h < 12 or n_w < 12:
                break
            map_H = int(np.floor((n_h - 12.) / 2)) + 1
            map_W = int(np.floor((n_w - 12.) / 2)) + 1
            tanchors.append(self.anchors[:map_H, :map_W].contiguous().view(-1, 4) / self.scale ** i)
            i += 1

        tanchors = torch.cat(tanchors, dim=0)
        ...
        xx = (xx - 127.5) / 128.0
        P_net_logits, P_net_loc, P_net_landmarks = self.model(xx)
        P_net_logits = P_net_logits.permute(0, 2, 3, 1).view(-1, 2)
        P_net_loc = P_net_loc.permute(0, 2, 3, 1).view(-1, 4)
        P_net_landmarks = P_net_landmarks.permute(0, 2, 3, 1).view(-1, 10)
...
def train(P_train, config, step, x, model_file=None):
    ...
    for epoch in range(epochs):
        for imgs, bboxes, num_b, num_H, num_W in dataloader:
            ...
            loss = list(map(P_train.get_train_data, imgs, bboxes, num_b, num_H, num_W))
            loss = list(zip(*loss))
            ...
            if (step <= 10000 and step % 1000 == 0) or step % 5000 == 0 or step == 1:
                torch.save(model.state_dict(), './models/P_net_%d_1.pth' % step)
            if step >= 90010 * x:
                flag = True
                break
        if flag:
            break
    torch.save(model.state_dict(), './models/P_net_final_1.pth')
...
if __name__ == "__main__":
    ...
    x = 0.5
    model_file = ''
    P_train = P_net_train(config)
    train(P_train, config, step, x, model_file=model_file)
